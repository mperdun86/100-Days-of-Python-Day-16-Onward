...
game_active = True
while game_active:
    user_guess = screen.textinput(title=f'{len(guessed_states)}/50 States guessed', prompt='Type the name of a U.S. State!').title()
    if user_guess == 'Exit':
        for state in state_names:
            if state not in guessed_states:
                need_to_learn.append(state)
        to_learn = pandas.DataFrame(need_to_learn)
        to_learn.to_csv('states_missed.csv')
        break
